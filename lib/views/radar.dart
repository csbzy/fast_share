...
class RadarPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _paint.shader = SweepGradient(
      colors: [const Color(0xffEDE75D).withOpacity(0.01), const Color(0xffEDE75D).withOpacity(0.5)],
      startAngle: 0.0,
      endAngle: pi / 4,
    ...
    canvas.drawArc(
        Rect.fromCircle(center: Offset(size.width / 2, size.height / 2), radius: radius), 0, pi / 4, true, _paint);
    canvas.restore();
  }
...
class RadarScan extends StatefulWidget {
  const RadarScan({super.key});

  @override
  _RadarViewState createState() => _RadarViewState();
}

class _RadarViewState extends State<RadarScan> with SingleTickerProviderStateMixin {
